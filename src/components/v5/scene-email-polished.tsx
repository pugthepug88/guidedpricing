...
function SmoothCarousel({ reduced }: { reduced: boolean }) {
    ...
    <div className="relative h-[206px] w-full">
      {!reduced ? (
        <motion.div
          ...
          animate={{ opacity: [0, 1, 1, 0], scale: [0.94, 1, 1, 0.9] }}
          transition={{ duration: 1.08, times: [0, 0.05, 0.82, 1], ease: [0.16, 0.84, 0.25, 1] }}
        >
          <motion.div
            className="absolute inset-0"
            style={{ transformStyle: "preserve-3d" }}
            initial={{ rotateY: 0 }}
            animate={{ rotateY: [0, 330, 420, 440] }}
            transition={{ duration: 1.08, times: [0, 0.51, 0.84, 1], ease: [0.16, 0.84, 0.25, 1] }}
          >
      ...
      <div className="absolute inset-x-0 top-2 flex justify-center gap-2.5">
        {TEMPLATES.map((_, index) => (
          <motion.div
            ...
            initial={reduced ? false : { opacity: 0, x: centerShift[index], y: 6, scale: 0.74, rotate: index === 0 ? -3 : index === 2 ? 3 : 0 }}
            animate={{ opacity: 1, x: 0, y: 0, scale: 1, rotate: 0 }}
            transition={{
              delay: reduced ? 0 : 0.78 + index * 0.02,
...
function CarouselOverlay({ phase, reduced }: { phase: number; reduced: boolean }) {
  return (
    <AnimatePresence>
      {phase === 3 ? (
        <motion.div
          className="absolute right-[1%] top-[14%] z-[70] h-[58%] w-[49%] bg-[#f7f8fb]"
        ...
        >
          ...
          <div className="relative mt-3.5 h-[206px]">
            <SmoothCarousel reduced={reduced} />
          </div>
        </motion.div>
      ) : null}
    </AnimatePresence>
...
export function SceneEmailPolished(props: SceneProps) {
  ...
  const suppressBaseMotion = phase <= 3;
  ...
  return (
    <div className="absolute inset-0 overflow-hidden">
      <SceneEmailLive {...props} reduced={suppressBaseMotion ? true : reduced} />
      <OpeningComposer phase={phase} reduced={reduced} />
      <CarouselOverlay phase={phase} reduced={reduced} />
      <ZaplaDemoCursor
        point={openingPoints[phase] ?? null}
        press={phase === 1}
        reduced={reduced}
      />
    </div>
