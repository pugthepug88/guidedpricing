...
const TEMPLATES = [
  { name: "Quote ready", image: photoA, headline: "Your quote is ready", button: "View quote", surface: "bg-white" },
  { name: "Fresh offer", image: photoB, headline: "A little something for you", button: "See offer", surface: "bg-amber-50" },
  { name: "News update", image: photoC, headline: "What’s new this month", button: "Read more", surface: "bg-emerald-950" },
] as const;
...
function TemplateVisual({ image, headline, button, surface, compact = false }: {
  image: string;
  headline: string;
  button: string;
  surface: string;
  ...
  return (
    <div className={`h-full w-full overflow-hidden rounded-[10px] ${surface}`}>
      ...
      <div className={compact ? "px-1.5 py-1.5" : "px-3 py-3"}>
        <div className={compact ? `text-[5px] font-black leading-tight ${dark ? "text-white" : "text-slate-900"}` : `text-[9px] font-black leading-tight ${dark ? "text-white" : "text-slate-900"}`}>
          {headline}
        </div>
        ...
        {!compact ? <div className="mt-3 inline-flex rounded-[7px] bg-zapla-blue px-3 py-1.5 text-[6.5px] font-black text-white">{button}</div> : null}
      </div>
    </div>
  );
}
...
function SequenceCard({ index, phase, reduced }: { index: number; phase: number; reduced: boolean }) {
  ...
  return (
    <motion.div
      className={selected ? "relative min-h-[76px] overflow-hidden rounded-[13px] border-2 border-blue-400 bg-white shadow-[0_16px_36px_-28px_rgba(37,99,255,.72)]" : "relative min-h-[76px] overflow-hidden rounded-[13px] border border-slate-200 bg-white shadow-[0_10px_28px_-25px_rgba(15,23,42,.42)]"}
      animate={{
    ...
    >
      <div className="flex min-h-[76px] items-center gap-2.5 p-2.5">
        <div className="flex h-[52px] w-[39px] shrink-0 items-center justify-center overflow-hidden rounded-[8px] bg-slate-100">
          {hasTemplate ? (
            index === 0 ? (
              <motion.div layoutId={SELECTED_LAYOUT_ID} className="h-full w-full" transition={{ type: "spring", stiffness: 125, damping: 23, mass: 0.9 }}>
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-1.5 flex items-center gap-1 text-[6.5px] font-bold text-slate-400"><Clock3 className="h-2.5 w-2.5" /> {email.delay}</div>
        </div>
...
function SequenceRail({ phase, reduced }: { phase: number; reduced: boolean }) {
  ...
  return (
    <motion.div className="relative z-20 flex h-full w-[31%] min-w-0 flex-col border-r border-slate-200 bg-white px-3 py-3">
      ...
      <div className="relative mt-4 flex-1">
        ...
        <div className="relative z-10 space-y-3.5">
          {EMAILS.map((email, index) => <SequenceCard key={email.title} index={index} phase={phase} reduced={reduced} />)}
        </div>
      </div>

      <motion.div
        className={active ? "mt-3 flex items-center justify-center gap-1.5 rounded-[10px] bg-emerald-500 px-3 py-2.5 text-[7.5px] font-black text-white" : "mt-3 flex items-center justify-center gap-1.5 rounded-[10px] bg-zapla-ink px-3 py-2.5 text-[7.5px] font-black text-white"}
...
function EmailWorkspace({ phase, reduced }: { phase: number; reduced: boolean }) {
  const open = phase >= 1 && phase <= 3;
  const showTemplates = phase >= 2 && phase <= 3;
        ...
        >
          ...
          <div className="relative h-[calc(100%-48px)] overflow-hidden bg-[#f7f8fb] px-4 py-4">
            <AnimatePresence mode="wait" initial={false}>
              {!showTemplates ? (
                <motion.div key="intro" className="flex h-full items-center justify-center" initial={reduced ? false : { opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0, y: -8 }}>
                  <div className="w-[78%] max-w-[420px] overflow-hidden rounded-[14px] border border-dashed border-slate-300 bg-white p-5 text-center">
                    <div className="mx-auto flex h-12 w-12 items-center justify-center rounded-[14px] bg-violet-50 text-violet-600"><Mail className="h-5 w-5" /></div>
                ...
                <motion.div key="templates" className="grid h-full grid-cols-3 gap-3" initial={reduced ? false : { opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={reduced ? undefined : { opacity: 0 }} transition={{ duration: reduced ? 0 : 0.32 }}>
                  {TEMPLATES.map((template, index) => {
                    ...
                    return (
                      <motion.div
                        ...
                        animate={{ opacity: dim ? 0.34 : 1, y: isSelected ? -3 : 0, scale: isSelected ? 1.025 : dim ? 0.97 : 1 }}
                        transition={{ duration: reduced ? 0 : 0.32, delay: reduced ? 0 : index * 0.07 }}
                      >
                        {index === 0 ? (
                          <motion.div
                            layoutId={SELECTED_LAYOUT_ID}
                            className={isSelected ? "relative min-h-0 flex-1 overflow-hidden rounded-[14px] border-2 border-blue-400 bg-white p-[3px] shadow-[0_22px_50px_-28px_rgba(37,99,255,.74)]" : "relative min-h-0 flex-1 overflow-hidden rounded-[14px] border border-slate-200 bg-white p-[3px] shadow-[0_18px_44px_-30px_rgba(15,23,42,.46)]"}
                            transition={{ type: "spring", stiffness: 125, damping: 23, mass: 0.9 }}
                          >
                            <TemplateVisual {...template} />
                            <AnimatePresence>
                              {isSelected ? (
                                <motion.span initial={reduced ? false : { opacity: 0, scale: 0.55 }} animate={{ opacity: 1, scale: 1 }} className="absolute right-2 top-2 flex h-6 w-6 items-center justify-center rounded-full bg-blue-600 text-white shadow-md">
                          ...
                          <div className="min-h-0 flex-1 overflow-hidden rounded-[14px] border border-slate-200 bg-white p-[3px] shadow-[0_18px_44px_-30px_rgba(15,23,42,.46)]"><TemplateVisual {...template} /></div>
                        )}
...
function QuietWorkspace({ phase, reduced }: { phase: number; reduced: boolean }) {
  ...
  return (
    <motion.div className="absolute inset-0 flex items-center justify-center bg-[#f7f8fb]" initial={reduced ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: reduced ? 0 : 0.35 }}>
      <motion.div
        className="pointer-events-none h-[72%] w-[72%] rounded-[24px]"
        animate={{ background: phase >= 6 ? "radial-gradient(circle at 28% 42%, rgba(16,185,129,.10), transparent 45%)" : "radial-gradient(circle at 28% 42%, rgba(37,99,255,.08), transparent 45%)" }}
      />
    </motion.div>
  );
}
  ...
  return (
    <LayoutGroup id="email-marketing-sequence">
      <div className="absolute inset-0 flex min-h-0 overflow-hidden bg-[#f7f8fb]">
        ...
        <ZaplaDemoCursor point={points[phase] ?? null} press={phase === 1 || phase === 3 || phase === 5} reduced={reduced} />
      </div>
    </LayoutGroup>
  );
}
