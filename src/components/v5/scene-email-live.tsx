...
function IconBadge({ children, small = false }: { children: React.ReactNode; small?: boolean }) {
  return (
    <span
      className={
        small
          ? "relative flex h-8 w-8 shrink-0 items-center justify-center overflow-hidden rounded-[10px] text-white shadow-[0_10px_20px_-10px_rgba(37,99,235,.9)]"
          : "relative flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-[12px] text-white shadow-[0_14px_28px_-12px_rgba(37,99,235,.9)]"
      }
...
function CarouselThumb({ index }: { index: number }) {
  ...
  return (
    <div
      className="relative h-[158px] w-[118px] overflow-hidden rounded-[18px] border-[3px] border-white bg-white shadow-[0_24px_46px_-24px_rgba(15,23,42,.52)]"
      ...
      <div className="flex justify-center gap-2.5">
        {TEMPLATES.map((_, index) => (
          <motion.div
            ...
            className="h-[190px] w-[144px] overflow-hidden rounded-[20px] border-[3px] border-white bg-white shadow-[0_26px_58px_-30px_rgba(15,23,42,.52)]"
          >
            <TemplateArtwork index={index} />
          </motion.div>
        ))}
  ...
  return (
    <div className="relative h-[206px] w-full">
      <motion.div
        ...
        animate={{ opacity: [0, 1, 1, 0], scale: [0.9, 1, 0.98, 0.8] }}
        transition={{ duration: 0.94, times: [0, 0.06, 0.78, 1], ease: [0.16, 0.84, 0.25, 1] }}
      >
        <motion.div
          className="absolute inset-0"
          style={{ transformStyle: "preserve-3d" }}
          initial={{ rotateY: 0 }}
          animate={{ rotateY: [0, 610, 690] }}
          transition={{ duration: 0.94, times: [0, 0.72, 1], ease: [0.16, 0.84, 0.25, 1] }}
        >
      ...
      <div className="absolute inset-x-0 top-2 flex justify-center gap-2.5">
        {TEMPLATES.map((_, index) => (
          <motion.div
            key={index}
            layoutId={`email-template-${index}`}
            className="h-[190px] w-[144px] overflow-hidden rounded-[20px] border-[3px] border-white bg-white shadow-[0_26px_58px_-30px_rgba(15,23,42,.52)]"
            initial={{ opacity: 0, x: centerShift[index], y: 6, scale: 0.72, rotate: index === 0 ? -4 : index === 2 ? 4 : 0 }}
            animate={{ opacity: 1, x: 0, y: 0, scale: 1, rotate: 0 }}
            transition={{
              delay: 0.92 + index * 0.025,
              duration: 0.38,
...
function TemplatePicker({ phase, reduced }: { phase: number; reduced: boolean }) {
  return (
    <AnimatePresence>
      {phase >= 3 && phase <= 6 ? (
        <motion.div
          className="absolute right-[1%] top-[14%] z-30 h-[58%] w-[49%]"
        ...
        >
          <div className="text-center">
            ...
            <div className="mt-0.5 text-[7px] font-semibold text-slate-400">Pick 3 emails for this sequence</div>
          </div>

          <div className="relative mt-3.5 h-[206px]">
            {phase === 3 ? (
              <FastCarousel reduced={reduced} />
            ) : (
              <div className="flex justify-center gap-2.5">
                {TEMPLATES.map((_, index) => (
        ...
        borderColor: active
          ...
          : incoming
            ...
            : filled
              ...
              : "rgba(226,232,240,1)",
        boxShadow: incoming
          ? "0 0 0 4px rgba(37,99,235,.06), 0 15px 34px -28px rgba(15,23,42,.42)"
          : "0 15px 34px -28px rgba(15,23,42,.42)",
      }}
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        {filled ? (
          ...
          <div className="mt-1.5 text-[7.5px] font-semibold text-slate-300">Template</div>
        )}
      </div>
      ...
      {filled ? (
        <motion.span
          ...
          className={
            active
              ? "flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#20bf5b] text-white shadow-[0_8px_18px_-8px_rgba(34,197,94,.8)]"
      ...
      <div className="flex h-full flex-col rounded-[22px] border border-slate-200 bg-white px-4 py-4 shadow-[0_24px_70px_-42px_rgba(15,23,42,.5)]">
        <div className="flex items-start justify-between">
          ...
          <span
            className={
              active
                ? "rounded-full bg-green-50 px-3 py-1.5 text-[7px] font-black text-green-700"
                : ready
                  ? "rounded-full bg-blue-50 px-3 py-1.5 text-[7px] font-black text-blue-700"
                  : "rounded-full bg-slate-100 px-3 py-1.5 text-[7px] font-black text-slate-500"
            }
          >
          ...
          <motion.div
            animate={
              active && !reduced
                ? {
                    boxShadow: [
                      "0 8px 22px -10px rgba(34,197,94,.5)",
                      "0 12px 30px -8px rgba(34,197,94,.75)",
                      "0 8px 22px -10px rgba(34,197,94,.5)",
                    ],
                  }
                : undefined
            ...
            className={
              active
                ? "inline-flex items-center gap-1.5 rounded-[11px] bg-[#16b857] px-5 py-2.5 text-[8.5px] font-black text-white"
                : ready
                  ? "inline-flex items-center gap-1.5 rounded-[11px] bg-[#18bd59] px-5 py-2.5 text-[8.5px] font-black text-white shadow-[0_12px_26px_-12px_rgba(34,197,94,.85)]"
                  : "inline-flex items-center gap-1.5 rounded-[11px] bg-slate-200 px-5 py-2.5 text-[8.5px] font-black text-slate-500"
            }
          >
            {active ? (
              <>
                <Check className="h-4 w-4" /> Campaign active
              </>
            ) : (
              <>
      ...
      {phase >= 9 ? (
        ...
        >
          ...
          <div className="relative rounded-[20px] bg-white p-5">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1fbd5b] text-white shadow-[0_10px_22px_-10px_rgba(34,197,94,.8)]">
