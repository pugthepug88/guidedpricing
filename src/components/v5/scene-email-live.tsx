              ...
              {!selected ? (
                ...
                >
                  {AUDIENCES.map(([name, count], index) => (
                    <motion.div
                      key={name}
                      className={index === 0 ? "flex items-center gap-2 rounded-[10px] bg-blue-50 px-2.5 py-2" : "flex items-center gap-2 rounded-[10px] px-2.5 py-2"}
                      animate={{ scale: phase === 1 && index === 0 ? 0.975 : 1 }}
                      transition={{ duration: reduced ? 0 : 0.14 }}
                    >
                      <span className={index === 0 ? "flex h-6 w-6 items-center justify-center rounded-full bg-blue-600 text-white" : "flex h-6 w-6 items-center justify-center rounded-full bg-slate-100 text-slate-400"}>
                        <Users className="h-3 w-3" />
                      </span>
                      <div className="flex-1">
...
function TemplateArtwork({ index, compact = false }: { index: number; compact?: boolean }) {
  ...
  if (compact) {
    const compactBg =
      t.tone === "peach"
        ? "linear-gradient(150deg,#ffb89d 0%,#f9735b 45%,#7f1d1d 100%)"
        : t.tone === "green"
          ? "linear-gradient(150deg,#b7e4c7 0%,#2f855a 48%,#12372f 100%)"
          : "linear-gradient(150deg,#bfdbfe 0%,#3b82f6 48%,#172554 100%)";
    return (
      <div className="relative h-full w-full overflow-hidden" style={{ background: compactBg }}>
        <img src={t.face} alt="" className="absolute bottom-0 right-[-5%] h-[90%] w-[78%] object-cover object-top opacity-95" />
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/35 via-transparent to-white/20" />
        <div className="absolute left-1.5 top-1.5 h-1.5 w-5 rounded-full bg-white/75" />
      </div>
    );
  }
  ...
  if (t.tone === "peach") {
    return (
      <div className="relative h-full w-full overflow-hidden rounded-[18px] bg-[#fff0ea]">
        <div className="absolute -right-10 -top-6 h-36 w-36 rounded-full bg-[#ffb39b]/70 blur-[2px]" />
        <div className="absolute -bottom-10 left-[-22px] h-28 w-28 rounded-full bg-[#f9735b]/18" />
        <img src={t.face} alt="" className="absolute bottom-0 right-[-8px] h-[72%] w-[58%] rounded-tl-[46px] object-cover object-top" />
        <div className="absolute left-3.5 top-3.5 rounded-full bg-white/80 px-2 py-1 text-[6px] font-black uppercase tracking-[.16em] text-[#c2412d]">{t.eyebrow}</div>
        <div className="absolute bottom-4 left-3.5 w-[56%] whitespace-pre-line text-[20px] font-black leading-[.9] tracking-[-.05em] text-[#7f1d1d]">{t.headline}</div>
      </div>
    );
  }
  ...
  if (t.tone === "green") {
    return (
      <div className="relative h-full w-full overflow-hidden rounded-[18px] bg-[#12372f]">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(255,255,255,.16),transparent_30%),radial-gradient(circle_at_90%_80%,rgba(74,222,128,.22),transparent_36%)]" />
        <img src={t.face} alt="" className="absolute right-[-10px] top-[12%] h-[78%] w-[58%] rounded-l-[44px] object-cover object-top opacity-95" />
        <div className="absolute left-3.5 top-3.5 rounded-full border border-white/20 bg-white/10 px-2 py-1 text-[6px] font-black uppercase tracking-[.16em] text-emerald-100 backdrop-blur-sm">{t.eyebrow}</div>
        <div className="absolute bottom-4 left-3.5 w-[55%] whitespace-pre-line text-[20px] font-black leading-[.9] tracking-[-.05em] text-white">{t.headline}</div>
      </div>
    );
  }

  return (
    <div className="relative h-full w-full overflow-hidden rounded-[18px] bg-[#eaf2ff]">
      <div className="absolute -left-8 -top-8 h-32 w-32 rounded-full bg-blue-400/18" />
      <div className="absolute right-[-12px] top-[-18px] h-32 w-32 rounded-full bg-indigo-500/20" />
      <img src={t.face} alt="" className="absolute bottom-0 right-[-6px] h-[74%] w-[58%] rounded-tl-[44px] object-cover object-top" />
      <div className="absolute left-3.5 top-3.5 rounded-full bg-blue-600 px-2 py-1 text-[6px] font-black uppercase tracking-[.16em] text-white">{t.eyebrow}</div>
      <div className="absolute bottom-4 left-3.5 w-[55%] whitespace-pre-line text-[20px] font-black leading-[.9] tracking-[-.05em] text-[#173a78]">{t.headline}</div>
    </div>
  );
}
...
function TemplatePicker({ phase, reduced }: { phase: number; reduced: boolean }) {
  return (
    <AnimatePresence>
      {phase >= 3 && phase <= 6 ? (
        <motion.div
          className="absolute right-[1%] top-[16%] z-30 w-[49%]"
        ...
        >
          <div className="mb-3.5 text-center">
            ...
            <div className="mt-0.5 text-[7px] font-semibold text-slate-400">Drag each email into the sequence</div>
          </div>
...
function DragCursor({ phase, reduced }: { phase: number; reduced: boolean }) {
  ...
  const index = phase - 4;
  const sourceLeft = [61.5, 76.5, 91][index];
  const targetTop = [39.5, 58.5, 77.3][index];
  ...
  return (
    <motion.div
      ...
      animate={{
        left: [`${sourceLeft}%`, `${sourceLeft}%`, "17.7%"],
        top: ["43%", "42%", `${targetTop}%`],
        opacity: [0, 1, 1],
        scale: [0.92, 0.87, 1],
      }}
    ...
    >
      ...
      />
      <svg width="30" height="30" viewBox="0 0 24 24" style={{ filter: "drop-shadow(0 1px 1.5px rgba(15,23,42,.45)) drop-shadow(0 6px 12px rgba(15,23,42,.25))" }}>
        <defs>
...
function SequenceRow({ index, phase, reduced }: { index: number; phase: number; reduced: boolean }) {
  ...
  return (
    <motion.div
      className="mx-auto flex h-[78px] w-[404px] max-w-[97%] items-center gap-3.5 rounded-[16px] border bg-white px-3.5 shadow-[0_15px_34px_-28px_rgba(15,23,42,.42)]"
      animate={{
        borderColor: active ? "rgba(34,197,94,.38)" : incoming ? "rgba(37,99,235,.42)" : filled ? "rgba(203,213,225,.95)" : "rgba(226,232,240,1)",
    ...
    >
      <div className="relative flex h-[68px] w-[54px] shrink-0 items-center justify-center overflow-visible rounded-[10px]">
        {!filled ? (
          ...
          <motion.div
            layoutId={`email-template-${index}`}
            className="h-full w-full overflow-hidden rounded-[10px] border-[2px] border-white bg-white shadow-[0_10px_22px_-14px_rgba(15,23,42,.5)]"
      ...
      <div className="min-w-0 flex-1">
        ...
        {filled ? (
          <motion.div initial={reduced ? false : { opacity: 0, y: 3 }} animate={{ opacity: 1, y: 0 }}>
            <div className="mt-1 truncate text-[9.8px] font-black text-slate-800">{TEMPLATES[index].subject}</div>
      ...
      {filled ? (
        <motion.span
          ...
          className={active ? "flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#20bf5b] text-white shadow-[0_8px_18px_-8px_rgba(34,197,94,.8)]" : "flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-blue-50 text-blue-600"}
        >
...
function Wait({ label }: { label: string }) {
  return (
    <div className="mx-auto flex w-fit items-center gap-1.5 rounded-full border border-slate-200 bg-white px-3 py-1.5 text-[7px] font-bold text-slate-500">
...
function Builder({ phase, reduced }: { phase: number; reduced: boolean }) {
  ...
  return (
    <div className="absolute bottom-[4%] left-[5%] top-[4%] z-10 w-[57%]">
      <div className="flex h-full flex-col rounded-[22px] border border-slate-200 bg-white px-4.5 py-4 shadow-[0_24px_70px_-42px_rgba(15,23,42,.5)]">
        <div className="flex items-start justify-between">
          <div className="flex items-center gap-3">
            <IconBadge><Megaphone className="h-5 w-5" /></IconBadge>
            <div>
              <div className="text-[13px] font-black text-slate-900">Customer Win-back</div>
              <div className="mt-0.5 text-[7.5px] font-semibold text-slate-400">3-email campaign</div>
            </div>
          </div>
          <span className={active ? "rounded-full bg-green-50 px-3 py-1.5 text-[7px] font-black text-green-700" : ready ? "rounded-full bg-blue-50 px-3 py-1.5 text-[7px] font-black text-blue-700" : "rounded-full bg-slate-100 px-3 py-1.5 text-[7px] font-black text-slate-500"}>
        ...
        <div className="mt-3.5 grid grid-cols-2 gap-3">
          <div className="rounded-[14px] border border-slate-200 bg-slate-50 px-3.5 py-3">
            ...
            {audience ? (
              <div className="mt-2.5 flex items-center justify-between gap-2">
                <AudienceChip />
                <span className="text-[7px] font-bold text-slate-400">312</span>
              </div>
            ) : (
              <div className="mt-2.5 text-[8px] font-semibold text-slate-300">Choose recipients</div>
            )}
          </div>
          ...
          <div className="rounded-[14px] border border-slate-200 bg-slate-50 px-3.5 py-3">
            ...
            <div className="mt-2.5 flex items-center justify-between">
              <span className="text-[9.5px] font-black text-slate-800">3 emails</span>
              <span className="text-[7.5px] font-bold text-slate-400">5 days</span>
            </div>
          </div>
        </div>
        ...
        <div className="mt-2 flex justify-center">
          <motion.div
            animate={active && !reduced ? { boxShadow: ["0 8px 22px -10px rgba(34,197,94,.5)", "0 12px 30px -8px rgba(34,197,94,.75)", "0 8px 22px -10px rgba(34,197,94,.5)"] } : undefined}
            transition={active && !reduced ? { duration: 1.8, repeat: Infinity } : undefined}
            className={active ? "inline-flex items-center gap-1.5 rounded-[11px] bg-[#16b857] px-5 py-2.5 text-[8.5px] font-black text-white" : ready ? "inline-flex items-center gap-1.5 rounded-[11px] bg-[#18bd59] px-5 py-2.5 text-[8.5px] font-black text-white shadow-[0_12px_26px_-12px_rgba(34,197,94,.85)]" : "inline-flex items-center gap-1.5 rounded-[11px] bg-slate-200 px-5 py-2.5 text-[8.5px] font-black text-slate-500"}
          >
            {active ? <><Check className="h-4 w-4" />Campaign active</> : <><Play className="h-4 w-4" />Activate campaign</>}
          </motion.div>
        </div>
      </div>
    </div>
...
function Summary({ phase, reduced }: { phase: number; reduced: boolean }) {
  return (
    <AnimatePresence>
      {phase >= 9 ? (
        <motion.div
          className="absolute right-[6%] top-[30%] z-20 w-[27%] overflow-hidden rounded-[22px] p-[2px] shadow-[0_30px_72px_-38px_rgba(15,23,42,.5)]"
          initial={reduced ? false : { opacity: 0, y: 14, scale: 0.96 }}
          animate={{ opacity: 1, y: 0, scale: 1 }}
        >
          <motion.div
            className="absolute -inset-[55%]"
            style={{ background: "conic-gradient(from 0deg,rgba(34,197,94,0),#22c55e,#60a5fa,rgba(34,197,94,0) 58%)" }}
            animate={reduced ? undefined : { rotate: 360 }}
            transition={reduced ? undefined : { duration: 2.6, repeat: Infinity, ease: "linear" }}
          />
          <div className="relative rounded-[20px] bg-white p-5">
            <span className="flex h-10 w-10 items-center justify-center rounded-full bg-[#1fbd5b] text-white shadow-[0_10px_22px_-10px_rgba(34,197,94,.8)]"><Check className="h-5 w-5" strokeWidth={3} /></span>
            <div className="mt-3 text-[13.5px] font-black text-slate-900">Campaign active</div>
