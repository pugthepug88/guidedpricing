...
function Sequence({ reduced, mobile }: { reduced: boolean; mobile: boolean }) {
  ...
  const filmBox = useBoxStyle(
    p,
    [0, 0.14, 0.34, 0.52, 0.58, 0.66, 0.86],
    mobile
      ? [
          [0, 0, 100, 52],
          [0, 0, 100, 52],
  ...
  const filmRadius = useTransform(p, [0.14, 0.34, 0.58, 0.66], [0, 10, 10, 8]);

  /* canvas: cinematic dark -> light product world ------------------- */
  const canvas = useTransform(p, [0.545, 0.59], ["#0A0E17", "#F5F6FA"]);
  ...
  const cueSurface = useTransform(p, [0.555, 0.6], [0, 1]);
  const cueInk = useTransform(p, [0.57, 0.6], ["#F7F8FC", INK]);
  const cueSub = useTransform(p, [0.57, 0.6], ["rgba(247,248,252,0.5)", "rgba(11,18,32,0.45)"]);
  const cueOpacity = useTransform(p, [0.7, 0.74], [1, 0]);
  const cueScale = useTransform(p, [0.6, 0.7], [1, 1.04]);

  /* statement -------------------------------------------------------- */
  const stmtOpacity = useTransform(p, [0.33, 0.37, 0.49, 0.525], [0, 1, 1, 0]);
  ...
  const shellOpacity = useTransform(p, [0.575, 0.605], [0, 1]);
  const headOpacity = useTransform(p, [0.7, 0.76], [0, 1]);
  const headY = useTransform(p, [0.7, 0.77], [14, 0]);
